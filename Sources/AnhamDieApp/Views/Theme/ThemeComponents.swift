...
struct PriorityBar: View {
    ...
    /// 비-모노 팔레트에서의 폭(회귀 유지) — 모노에서는 resolvedPriorityStyle의 농도·두께가 우선한다(§15.2).
    var width: CGFloat = 3
    var minHeight: CGFloat = 16

    var body: some View {
        let style = AppTheme.resolvedPriorityStyle(priority)
        let barWidth = AppTheme.palette.monochrome ? style.width : width
...
///
/// 시그니처는 v5 공용 계약(V5-Scaffold). 색은 resolvedTagColor 경유 — 모노 팔레트면 회색조,
/// 아니면 사용자 지정 hex 파싱(실패 시 비활성 텍스트색 폴백)(§15.2).
struct TagPill: View {
    let name: String
    let colorHex: String
    ...
    private var tint: Color {
        AppTheme.resolvedTagColor(hex: colorHex)
    }
