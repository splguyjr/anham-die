...
## 19. v10 요구사항 (2026-08-04 확정 — 포스트잇 스티키 노트)

### 19.1 개별 포스트잇 창

- 투두와 별개의 **포스트잇 메모** — 노트마다 독립 플로팅 카드 창(Stickies 스타일):
  자유 이동·리사이즈, 위치·크기·색상 영속, 텍스트 즉시 편집(멀티라인).
- 카드 UI: 상단 얇은 바(📌 핀 토글 · 색상 점 메뉴 · **+ 새 노트** · 닫기), 본문 텍스트 에디터.
  클래식 포스트잇 색 5종(노랑/핑크/민트/블루/라벤더) — 노트별 지정, 텍스트 대비 확보.
  포스트잇 색은 데이터 정체성이므로 모노 테마에서도 유지(무채 규칙 예외 명시).
- **핀(📌)**: 기본은 일반 창 레벨(다른 앱 뒤로 감), 핀 켜면 그 노트만 항상 위
  (오버레이처럼 전체화면 위 포함). 핀 상태 영속.
- + 버튼/단축키로 새 노트 — 커서(또는 기존 노트) 근처에 살짝 어긋나게 생성, 즉시 입력 포커스.

### 19.2 보관함 (기록)

- 노트 닫기 = 삭제가 아니라 **보관(archived)** — 완료 히스토리 철학과 동일.
- 메인 사이드바에 **'포스트잇' 뷰** 추가: 열린 노트 목록 + 보관된 노트(닫은 날짜별 그룹).
  항목에서 **다시 열기 / 완전 삭제**, 본문 미리보기 표시.

### 19.3 단축키·접근 경로

- **⌥⌘S = 새 포스트잇** (즉시 생성+포커스), **⌥⌘H = 열린 포스트잇 전체 표시/숨김 토글**.
  설정 > 단축키에 Recorder·개별 on/off 통합, JetBrains 예외·마스터 토글 적용.
- 메뉴바 팝오버에 "새 포스트잇" 버튼 추가.

### 19.4 저장·기술

- StickyNote 모델(id·text·colorID·frame·pinned·createdAt·archivedAt) —
  투두 스토어와 **분리된 파일**(stickies.json, 자체 버전+디바운스) 권장: 투두 변경 알림이
  노트 창을 재렌더하지 않게 관찰 분리. 손상 백업(.corrupt) 규약 동일 적용.
...
## 20. v11 요구사항 (2026-08-04 확정 — 주간 목표)

### 20.1 개념 — 논리적 주

- **논리적 주 = 월요일 기준시각(06:00) 시작** (주 시작 요일은 설정에서 변경 가능).
- WeeklyGoal: 제목 · 목표 횟수 n(1=단일형 체크) · 현재 진행 · **루틴 여부(매주 반복)** ·
  소속 주 · 상태. 예: "헬스 3회"(횟수형·루틴), "책 1챕터"(단일형), "코테 3문제"(횟수형).

### 20.2 일일 연계 (핵심)

- 주간 목표에서 **[오늘 하기]** → 목표에 연결된 task를 오늘에 생성(task에 weeklyGoalID,
  행에 ↗주간 배지). **연결 task 완료 확정(1.5초 유예 통과) 시 목표 진행 +1**,
  완료 취소 시 -1. 목표 패널에서 **직접 +1/-1도 병행** 가능.
- 연결 task는 일반 task와 동일하게 이월·백로그·삭제 가능(삭제해도 이미 오른 카운트는 유지).

### 20.3 주 전환 처리

- **루틴 목표**: 새 주 시작 시 자동으로 0/n 재생성(이전 주 기록은 히스토리 보관).
- **비루틴 미달 목표**: 새 주 **첫 브리핑의 '지난주 리뷰' 섹션**에서 달성/미달 요약과 함께
  [이번 주로 이월(잔여량만) / 보류 / 종료] 선택 — 일일 이월과 동일한 철학.
- 비루틴 목표를 나중에 **루틴으로 전환**(또는 해제)하는 토글 제공.

### 20.4 노출 (4곳)

- **사이드바 '주간 목표' 뷰**: 추가/편집(제목·횟수·루틴 토글)·진행 링·[오늘 하기]·직접 ±1·
  지난 주 기록(주별 그룹, 달성률).
- **해야할 일 상단 요약 카드**: 이번 주 목표 진행 스트립(접기 가능, 접힘 상태 영속).
- **캘린더 주간 뷰 헤더**: 그 주의 목표 진행 스트립(현재 주가 아니면 해당 주 기록).
- **브리핑**: 하단에 이번 주 진행 미니 요약, 월요일 첫 브리핑엔 '지난주 리뷰' 섹션.

### 20.5 저장·기술

- 목표는 store.json에 통합 저장 권장(task.weeklyGoalID와 밀접 — 한 번의 스키마 버전 업 +
  마이그레이션, .corrupt 백업 규약). DayBoundaryService에 논리적 주 계산 확장.
...
## 21. v12 요구사항 (2026-08-05 확정 — 2차 사용 피드백 다발)

조사로 확인한 현재 상태를 근거로 함(각 항목 finding 표기).

### 21.1 주간 목표 [오늘 하기] 토글
- **[오늘 하기]를 다시 누르면 해제**: 연결 task가 오늘에 존재하고 미완료면 제거 + (아직 카운트 안 오른 상태이므로) 목표 진행 불변. 이미 완료된 연결 task는 토글 대상 아님(카운트 유지). 버튼 상태를 "오늘 있음/없음"으로 반영(눌린 상태 표시).
- 적용: 사이드바 주간 목표 뷰 + §21.9 요약 카드 공통(공용 액션).

### 21.2 오버레이 드래그 위치 밀림 (버그)
- finding: OverlayView.swift mouseDragged가 `origin += (now-last)` 증분 누적 → 리사이즈 재측정(onGeometryChange 앵커)·클램프와 겹쳐 그랩 지점이 벌어짐.
- 수정: 드래그 시작 시 (mouseDownScreenLocation - window.frame.origin) 오프셋 1회 기록 → 매 이동은 **절대식** `origin = currentMouseLocation - grabOffset`. 리사이즈 억제 가드가 드래그 중 높이 재측정을 막는지 확인.

### 21.3 오버레이·브리핑 크기 조절
- finding: 오버레이는 이미 .resizable이나 borderless라 그립이 안 보임. 브리핑은 .resizable 자체가 없음.
- 오버레이: 우하단 **리사이즈 핸들 어포던스**(작은 그립) 표시 + 가장자리 히트영역 확보. 크기 저장·복원 유지.
- 브리핑: BriefingPanel에 .resizable + min/max + 크기 저장·복원(오버레이 패턴 준용), 콘텐츠 스크롤.

### 21.4 브리핑 주간 목표 가시성 + [오늘 하기]
- finding: 브리핑 주간 섹션은 요약(진행)만 있고 액션 없음.
- 가시성 개선: 이번 주 목표를 진행 바/링과 함께 또렷하게(축약 말고 읽히게), 지난주 리뷰와 구분.
- **각 목표에 [오늘 하기] 액션 추가**(§21.1 공용 토글 재사용). 브리핑에서 바로 오늘에 배치/해제.
- 안내: 자동 브리핑은 논리적 하루(06:00) 첫 활성화 시 1회 — 안 떴으면 ⌥⌘T로 즉시 호출 가능(문서/툴팁).

### 21.5 포스트잇 빈 내용 닫기 = 저장 안 함
- 카드 닫을 때 text가 공백만(trim 후 빈 문자열)이고 편집 이력이 없으면 **아카이브하지 않고 폐기**(stickies.json에 안 남김). 내용 있으면 기존대로 아카이브.

### 21.6 캘린더 주간 목표 노출 개선
- finding: CalendarWeekView 헤더에만 목표 칩 존재(월간/일간엔 없음). 기본 진입이 월간이라 사용자가 못 봄.
- 월간 뷰 헤더에도 이번 주(표시 중인 주가 현재 주면 라이브, 아니면 해당 주 기록) 목표 진행 스트립 추가. 주간 뷰 헤더 칩은 가시성 강화(진행 n/m 명확히). 스트립 클릭 → 주간 목표 뷰로 이동.

### 21.7 포스트잇 다중 선택 삭제
- 사이드바 '포스트잇' 뷰(보관 목록)에서 다중 선택(체크박스 또는 ⌘/⇧클릭) + "선택 삭제"(확인 후 완전 삭제). 열린 노트 목록도 다중 선택 닫기/삭제 지원 검토.

### 21.8 완료 뷰 '취소됨' → 오늘로 복구
- finding: reactivate(되돌리기)는 있으나 scheduledDate가 과거라 '지난 할 일'로 감. Done 뷰 취소됨 행에 오늘 복구 경로 부재.
- 취소됨(및 완료) 행에 **"오늘로 복구"** 액션 추가: reactivate + scheduledDate=오늘. 기존 "되돌리기"(제자리 되살리기)는 유지하되 취소됨엔 "오늘로 복구"를 기본 노출.

### 21.9 해야할 일 상단 요약 카드에서 [오늘 하기]
- 요약 카드의 각 목표를 클릭/버튼으로 **오늘 할일에 배치**(§21.1 공용 토글). 카드에서 바로 진행 시작.

### 21.10 캘린더 '지우기' 라벨 명확화
- finding: DueDateControls "지우기"(X)는 scheduledDate=nil → 백로그 이동인데 삭제로 오해됨.
- 라벨/툴팁을 동작에 맞게: "백로그로"(또는 "날짜 비우기 · 백로그") + 아이콘 변경(tray 등). 삭제와 시각적으로 구분. 동작 자체(백로그+rolloverCount 리셋)는 유지.
...
## 22. v13 요구사항 (2026-08-05 확정 — '언젠가' 리스트)

### 22.1 개념 — 백로그와 별개
- **'언젠가' 리스트**: Due 없이 여유될 때 의도적으로 꾸준히 해나가는 일들의 전용 목록.
- 백로그(보류·브리핑에서 밀려온 것)와 **성격/데이터 분리**: TodoTask에 bucket 구분
  (`bucket: active/backlog/someday` 권장) — 백로그=nil 일정 & backlog, 언젠가=nil 일정 & someday.
  store.json 스키마 버전 업 + 마이그레이션(기존 nil 일정 항목은 backlog로, .corrupt 규약).

### 22.2 노출 (3곳)
- **별도 사이드바 '언젠가' 뷰**: 추가/편집/수동정렬, 항목별 [오늘 하기]·해야할일로 드래그, 완료/삭제, 메모 미리보기 준용.
- **오버레이 '여유 칸'**: 오늘 할 일 아래 접이식 여유 섹션에 언젠가 몇 개 노출 —
  특히 오늘 할 일을 다 끝냈을 때 강조. 설정 on/off·표시 개수.
- **별도 글로벌 단축키**: 언젠가 패널 토글(기본 ⌥⌘L, 설정 변경·개별 토글·JetBrains 예외 적용).
  HotkeyService에 .toggleSomeday Name + 설정 단축키 탭 행.

### 22.3 오늘로 가져오기 / 복귀
- [오늘 하기] 버튼 또는 해야할일 리스트로 드래그 → 오늘 task(scheduledDate=오늘, someday origin 기억).
- **안 끝내면 다시 '언젠가'로**: someday-origin 미완료 task는 논리적 하루 경계에서 일반 이월/overdue가 아니라
  **언젠가로 복귀**(rolloverCount 안 오름 — 백로그 리셋 철학). 완료 시엔 일반처럼 히스토리.
