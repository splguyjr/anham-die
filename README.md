...
- **메인 창 — 사이드바형 (Things 스타일)** — 좌측 사이드바에 스마트 뷰 4개
  (**해야할 일 / 백로그 / 완료 / 캘린더**) + 태그 목록(색 점, 클릭 시 해당 태그로 필터).
  **사이드바 접기 (v4)**: 상단 토글로 사이드바를 접어 캘린더·리스트에 전폭 확보,
  접힘 상태 저장·복원.
  ...
  - 행: 체크박스 · 제목 · 우선순위/태그 색 점 · D-day 배지 · 이월 배지(↺n).
    D-day 색: 지남=빨강, 오늘=주황, 임박(≤3일)=노랑, 여유=회색.
  - `AppTheme` 단일 디자인 시스템 — 타이포 스케일·고대비 텍스트·호버 피드백·액센트 컬러.
